...
# 1. 페이지 설정
st.set_page_config(page_title="제이미 로또 31 - 데이터 보존 모드", layout="wide")
...
st.markdown("""
    <div style="text-align: center; border-bottom: 5px solid #ff4b4b; padding-bottom: 20px; margin-bottom: 30px; background-color: #fff5f5; border-radius: 15px;">
        <h1 style="margin: 0; color: #ff4b4b; font-size: 2.8rem; font-weight: 900;">🎰 제이미 로또 31 분석기</h1>
        <p style="color: #333; font-size: 1.3rem; font-weight: bold;">[ 1209회 대비 - 데이터 유지 모드 ]</p>
    </div>
""", unsafe_allow_html=True)

col1, col2 = st.columns([1, 1])

# --- [Step 1] 왼쪽: 자동 입력 (입력값 유지) ---
with col1:
    st.markdown("### 📥 <span style='font-size: 1.5rem;'>Step 1. **자동 10게임** 입력</span>", unsafe_allow_html=True)
    st.write("---")
    
    auto_all = []
    labels = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']
    
    # st.text_input의 key 값을 고정하여 세션 내내 값이 유지되도록 함
    for label in labels:
        val = st.text_input(f"**🎮 게임 {label}**", placeholder="번호 입력 (예: 1, 2, 3...)", key=f"stable_auto_{label}")
        if val:
...
# --- [Step 2] 오른쪽: 전략 대입 (입력값 유지) ---
with col2:
    st.markdown("### 🎯 <span style='font-size: 1.5rem;'>Step 2. **전략 번호** 대입</span>", unsafe_allow_html=True)
    st.write("---")
    
    # 전략 입력창도 세션 유지를 위해 고정 key 사용
    user_core = st.text_input("💎 **핵심 전략 (7구+)**", placeholder="내일 고수 추천 상위 번호", key="stable_core")
    user_support = st.text_input("🌿 **보조 소외 (10구+)**", placeholder="보험용 번호 입력", key="stable_support")
    ...
    if matched_c: st.markdown(f"#### ✅ **매칭 핵심수**: <span style='color:#ff4b4b; font-size:1.4rem;'>{matched_c}</span>", unsafe_allow_html=True)
    if matched_s: st.markdown(f"#### ✅ **매칭 소외수**: <span style='color:#007bff; font-size:1.4rem;'>{matched_s}</span>", unsafe_allow_html=True)
